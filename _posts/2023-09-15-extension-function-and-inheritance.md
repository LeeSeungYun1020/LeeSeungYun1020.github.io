...
excerpt: "상속 관계에서 확장 함수를 사용하면 위험한 이유"
---

## 들어가기

interface나 상속할 수 있는 open class에 확장 함수를 정의할 때는 주의가 필요하다.  
확장 함수는 정적 디스패치(static dispatch) 방식으로 동작하기 때문에, 컴파일 시점에 리시버 타입을 이용하여 어떤 함수가 호출될지 결정된다.  
...
아마 위 예시에서 리스트에 Rectangle을 담았을 때, "Rectangle"을 반환할 것을 기대했을 것이다.  
실행 결과는 "Shape"를 출력하는데, 확장 함수는 정적 디스패치라 선언된 타입에 따라 호출할 함수가 컴파일 시 미리 결정되기 때문이다.  
이와 같이 하위 클래스에 확장 함수를 정의하였더라도 컬렉션에서 상위 타입으로 참조하는 경우, 하위 클래스를 리시버로 받은 확장 함수가 호출되지 않는다.
...
동일한 시그니처로 멤버 함수와 확장 함수를 정의하면, 멤버 함수가 우선 호출된다.  
확장 함수는 클래스나 인터페이스를 수정하는 것이 아니라는 설계 철학이 그대로 반영된 것이다.
설령 다음 코드를 추가하더라도 멤버 함수에 정의된 대로 A, B, C 각각이 정상적으로 출력된다.  
...
이는 확장 함수가 컴파일 타임에 선언된 변수 타입에 따라 정적으로 디스패치 되는 것과 다르게,
멤버 함수는 런타임에 실제 객체 타입에 따라 동적으로 호출되기 때문이다.  
이러한 특성 때문에 확장 함수는 외부 라이브러리 클래스에 유틸리티 기능을 추가할 때 많이 사용된다.  

## 방법 2: 최상위 클래스에만 확장 함수 정의

시스템이나 외부 라이브러리에 선언된 클래스라 멤버 함수 정의가 불가능한 클래스라면 어떻게 해야할까?  
